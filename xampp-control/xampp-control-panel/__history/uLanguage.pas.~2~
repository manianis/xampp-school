...
uses
  ...
  Controls, Forms,
  Dialogs, StdCtrls, Buttons, jpeg, ExtCtrls, Registry, Vcl.Imaging.pngimage;
...
begin
  ...
  if Config.Language = 'en' then
    ...
    rbDe.Checked := true
  else if Config.Language = 'fr' then
    rbFr.Checked := true
  else
    rbEn.Checked := true;
end;
...
begin
  ...
  if (Sender = rbDe) or (Sender = ImgDe) then
    rbDe.Checked := true;
  if (Sender = rbFr) or (Sender = ImgFr) then
    rbFr.Checked := true;
end;
