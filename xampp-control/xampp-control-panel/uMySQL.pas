...
uses GnuGettext, uBaseModule, SysUtils, Classes, Windows, ExtCtrls, StdCtrls,
  Buttons, uNetstatTable, uTools, uProcesses_new, uServices, uUtils;
...
type
  tMySQL = class(tBaseModule)
    ...
    constructor Create;
    // (pbbService: TBitBtn; pStatusPanel: tPanel; pPIDLabel, pPortLabel: tLabel; pStartStopButton, pAdminButton: TBitBtn);
...
var
  App, Param: string;
begin
  RunInBrowser('/phpmyadmin/', Config.ServicePorts.Apache, Config.BrowserApp,
    Self.AddLog);
end;
...
begin
  ...
  AddLog(Format(_('Checking for service (name="%s"): %s'),
    [Config.ServiceNames.MySQL, s]), ltDebug);
  if (path <> '') then
  begin
    if (Pos(LowerCase(basedir + 'mysql\bin\' + Config.BinaryNames.MySQL),
      LowerCase(path)) <> 0) then
      AddLog(Format(_('Service Path: %s'), [path]), ltDebug)
    else
    begin
      // pStatus.Color := cErrorColor;
      statusColor := cErrorColor;
      AddLog(Format(_('%s Service detected with wrong path'), [cModuleName]
        ), ltError);
      AddLog(Format(_('Change XAMPP %s and Control Panel settings or'),
        [cModuleName]), ltError);
      AddLog(_('Uninstall/disable the other service manually first'), ltError);
      AddLog(Format(_('Found Path: %s'), [path]), ltError);
      AddLog(Format
        (_('Expected Path: %smysql\bin\%s --defaults-file=%smysql\bin\my.ini %s'),
        [basedir, Config.BinaryNames.MySQL, basedir, Config.ServiceNames.MySQL]
        ), ltError);
    end
  end
  else
...
begin
  ...
  begin
    // pStatus.Color := cErrorColor;
    statusColor := cErrorColor;
    AddLog(Format(_('Problem detected: %s Not Found!'), [cModuleName]),
      ltError);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AddLog(Format(_('XAMPP %s is already running on port %d'),
          [cModuleName, ServerPort]), ltInfo);
      end
      // else if (Pos(LowerCase(PortBlocker), LowerCase(path)) <> 0) and (isService = True) then
      else if (Pos(LowerCase(pbspath), LowerCase(path)) <> 0) and
        (isService = True) and (Pos(LowerCase(ServerApp), LowerCase(pbspath))
        <> 0) then
      begin
        AddLog(Format(_('XAMPP %s Service is already running on port %d'),
          [cModuleName, ServerPort]), ltInfo);
      ...
      begin
        ...
        AddLog(Format(_('Port %d in use by "%s"!'), [ServerPort, PortBlocker]
          ), ltError);
        AddLog(Format(_('%s WILL NOT start without the configured ports free!'),
          [cModuleName]), ltError);
        AddLog(_('You need to uninstall/disable/reconfigure the blocking application'),
          ltError);
        AddLog(Format
          (_('or reconfigure %s and the Control Panel to listen on a different port'),
          [cModuleName]), ltError);
      end;
    end;
  end;
...
begin
  ...
  Param := '--install "' + Config.ServiceNames.MySQL + '" --defaults-file="' +
    basedir + 'mysql\bin\my.ini"';
  ...
  if RC = 0 then
    ...
    AddLog(Format(_('There may be an error, return code: %d - %s'),
      [RC, SystemErrorMessage(RC)]), ltError);
end;
...
begin
  ...
  begin
    ...
    App := '"' + basedir + 'mysql\bin\' + Config.BinaryNames.MySQL +
      '" --defaults-file="' + basedir + 'mysql\bin\my.ini" --standalone';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((Pos(IntToStr(Config.ServicePorts.MySQL),
          NetStatTable.GetPorts4PID(currPID)) <> 0) and
          (Pos(LowerCase(basedir), ppath) <> 0)) or
  ...
  begin
    ...
    begin
      ...
      begin
        // pStatus.Color := cErrorColor;
        statusColor := cErrorColor;
        AddLog(Format(_('Error: %s shutdown unexpectedly.'), [cModuleName]
          ), ltError);
        AddLog(_('This may be due to a blocked port, missing dependencies, '),
          ltError);
        AddLog(_('improper privileges, a crash, or a shutdown by another method.'),
          ltError);
        AddLog(_('Press the Logs button to view error logs and check'),
          ltError);
    ...
    if isRunning then
      statusColor := cRunningColor
    else
      statusColor := cStoppedColor;
  end;
  ...
  begin
    ...
    if isRunning then
    begin
      AddLog(Format(_('Autostart aborted: %s is already running'), [cModuleName]
        ), ltInfo);
    end
    else
    begin
